...
function StateFields(props: IStateFieldsProperties): ReactElement {
  return (<>
    ...
    {props.items.map((item, index) => props.remove === index ?
      <Fragment key={index}>
        <input type="hidden" name={`items[${index}][planGUID]`} value={item.planGUID} />
        <input type="hidden" name={`items[${index}][numberOfNodes]`} value={item.numberOfNodes} />
        <input type="hidden" name={`items[${index}][memoryInMB]`} value={item.memoryInMB} />
...
function Plans(props: IPlansProperties): ReactElement {
  return (<>
    {values(mapValues(groupBy(props.plans, 'version'), (plans, version) => (
          ...
          <form className="paas-service-selection" method="get">
            ...
            {plans.length > 1 ?
              <div className="govuk-form-group">
                <label className="govuk-label" htmlFor={`service-${props.serviceName}-${version}`}>
                  Sort by
                </label>
                <select
                ...
                >
                  {plans.map(plan =>
                    <option key={plan.planGUID} value={plan.planGUID}>{plan.planName}</option>)}
                </select>
              </div>
              :
              <>
                <input type="hidden" name={`items[${props.state.items.length}][planGUID]`} value={plans[0].planGUID} />
                {props.serviceName === 'app' ?
                  <>
                    <div style={{width: '46%'}}>
                      <label className="govuk-label" htmlFor={`nodes-${props.serviceName}-${version}`}>
                        Sort by
                      </label>
                      <select
                        className="govuk-select govuk-!-width-full"
                    ...
                    <div style={{ float: 'left', marginLeft: '14px', width: '46%' }}>
                      <label className="govuk-label" htmlFor={`mem-${props.serviceName}-${version}`}>
                        Sort by
                      </label>
                      <select
                        className="govuk-select govuk-!-width-full"
            ...
            <button type="submit" className="paas-link-button">+ Add</button>
          </form>
        </td>
      </tr>
    )))}
    ...
    <div className="govuk-grid-row">
      <div className="govuk-grid-column-one-half">
        ...
        <p className="paas-calculator-lede">
          Service teams usually run 3 environments, such as:
          {' '}
          <br />
          {' '}
      ...
      <div className="govuk-grid-column-two-thirds paas-service-list">
        <table className="govuk-table">
          ...
          <tbody className="govuk-table__body">
            {values(mapValues(groupBy(props.state.plans, 'serviceName'), (plans, serviceName) => (<Plans
              key={serviceName}
              plans={plans}
      ...
      <div className="govuk-grid-column-one-third paas-summary-section">
        ...
        <table>
          ...
          {props.quote.events.map((event, index) => (
            <Fragment key={index}>
              <tr>
                <td className="paas-service-heading">
                  {event.resourceType}
                </td>
                <td>
                  <form method="get">
                    <StateFields
                      ...
                      remove={index}
                    />
                    <button className="paas-remove-button" type="submit" arria-label="Remove">&times;</button>
                  </form>
                </td>
              </tr>
              <tr className="paas-service-items">
                <td>
                  {event.resourceType === 'app' && event.numberOfNodes > 0 ? `${event.numberOfNodes} Instances` : ''}
                  {event.resourceType === 'app' && event.memoryInMB > 0 ? `${event.memoryInMB} MB` : ''}
                  {event.resourceType !== 'app' ? event.resourceName : ''}
                </td>
                <td className="paas-service-price">£{event.price.exVAT.toFixed(2)}</td>
              </tr>
            </Fragment>
          ))}
          {props.quote.exVAT > 0 ?
            <tr className="paas-admin-fee">
              <td>admin fee 10%</td>
              <td className="paas-service-price">£{(props.quote.exVAT / 100 * 10).toFixed(2)}</td>
            </tr> : <></>}
        </table>

        <p className="paas-total">Estimated monthly cost</p>

        <p className="paas-price">£{(props.quote.exVAT + props.quote.exVAT / 100 * 10).toFixed(2)}</p>
        <p className="paas-month">per month</p>

        <details className="govuk-details">
          <summary className="govuk-details__summary">
            <span className="govuk-details__summary-text">Why costs may vary</span>
          </summary>
          <div className="govuk-details__text">
            <p>The estimate assumes constant usage at the levels provided by you.</p>
            <p>
              CPU/Postgres and MySQL are charged based on hourly consumption and may change based on exchange rates.
            </p>
            <p>Valid {props.state.monthOfEstimate}. Estimates are subject to change.</p>
          </div>
        </details>
      </div>
    </div>
